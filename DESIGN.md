...
> **Approved layouts** are locked via `/design-shotgun` (2026-06-17). Mockups live in `~/.gstack/projects/jhill247-MadeInAmericas/designs/<screen>/APPROVED-*.png`. Build to the approved layout + the build notes; do not re-litigate structure.

1. **Home (product-first)** — **[APPROVED: A2]** `designs/homepage-search-20260617/APPROVED-homepage.png`. TopNav · search hero (navy band, map motif) · **category grid** (verticals + counts) · trust strip · **Hot/Trending products** grid (`ProductCard`) · Cost Estimator + Browse-by-Country `EntryTile`s · SEO Footer.
2. **Vertical / department landing** (`/v/[verticalSlug]`) — **[APPROVED: C, editorial-guide]** `designs/vertical-landing-20260617/APPROVED-vertical-landing.png`. Vertical hero + intro guide copy · sub-category chip row (build note: may promote to a tile grid for browse weight) · trending products · **ranked Top-manufacturers `ComparisonTable`** (Rank / Verification / Response tier / MOQ / Price + Request Quote) · "Why source in the Americas" editorial · scoped Cost Estimator band · deep FAQ (schema-rich) · SEO footer.
...
5. **Manufacturer profile** (`/m/[slug]`) — **[APPROVED: A, profile-classic]** `designs/manufacturer-profile-20260617/APPROVED-manufacturer-profile.png`. Sticky **left trust sidebar** (logo, `VerificationBadge`, circular `ResponseScoreChip`, location, years, cert chips, **Request Quote** + Message) + right **tabbed main** (Overview / Capabilities / Certifications / Products / Facility / Quality / Documents / Reviews) with About, Key Facts grid, facility photos, products preview. Build notes: apply tri-state `PriceIndicator` on product cards; enrich sidebar/Overview with credibility depth (response-rate trend, `CompletenessMeter`, verified-document chips) from the trust-dossier variant; keep nav per §5 TopNav.
6. **Product listing** (`/m/[slug]/p/[productSlug]`) — **[APPROVED: C, quote-flow]** `designs/product-listing-20260617/APPROVED-product-listing.png`. Alibaba-style (PLAN §8): left media gallery + right **"Build your RFQ" configurator** (Quantity, Variation, Customization toggles) with a **live tri-state Estimated unit-cost range** tied to the Cost Estimator (§24) + line-item link, primary **Request Quote** + secondary **Request Sample**; below: specs · certifications (verified) · supplier snapshot · FAQ · related products. Build notes: retain the deeper specifications + COA/heavy-metals/microbial table (datasheet variant) and ensure all §8 sections (variations/SKUs, MOQ & lead-time, packaging & delivery, samples) are present.
...
## Changelog
- **2026-06-17 (2):** Locked 3 more page-template layouts via `/design-shotgun` (all on the A2 navy/slate system): **vertical landing → C (editorial-guide)**, **manufacturer profile → A (profile-classic)**, **product listing → C (quote-flow)**. Annotated §6 with `[APPROVED]` markers, approved-mockup paths, and per-screen build notes.
